   ...
   _.filter(this.Nodes, function(Node) {
    if(Node.class ==="Playbook" && Node.props._oid){
        ...
        if((reviewConceptName.toUpperCase()).trim()==Node.props.name.toUpperCase())
        {
           ...
           window.open("https://mattersmith4.embeddedexperience.com/#playbook_openOid="+Node.props._oid+"&filter_includeManual="+Node.props._oid+"&select="+Node.props._oid+"&v=1&path="+Node.props.attachment)
